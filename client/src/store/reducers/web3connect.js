...
import { fetchUserTrees } from '../actions';
...
export const sync = () => async (dispatch, getState) => {
	...
	const { account, watched, contracts, networkId, transactions: { pending } } = getState().web3connect;
	...
	if (account !== accounts[0]) {
		...
		dispatch(fetchUserTrees(accounts[0]));
	}
